...
app.get("/v1/zillow/zestimate", (request, response) => {
  ...
  values = lo.map(values, (v) => {
    return lo.parseInt(v);
  });
  zestimate =
    lo.reduce(values, (total, v) => {
      return (total *= v);
    }) * 10;
...
app.get("/v1/zillow/houses", (request, response) => {
  ...
  var resultHouses = lo.filter(houses, (house) => {
    return house.city === searchCity;
  });
...
app.get("/v1/zillow/prices", (request, response) => {
  ...
  const housesUnderPrice = lo.filter(houses, (house) => {
